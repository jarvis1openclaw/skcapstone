...
class ConnectorCapabilities:
    ...
    def validate(self, expected_identity: str) -> None:
        ...
        expected = FORGEJO_REVIEW_PERMISSIONS if self.forge == "forgejo" else REVIEW_PERMISSIONS
        if self.forge not in {"github", "forgejo"} or self.permissions != expected:
            raise ReviewPublicationError("connector_permissions_not_least_privilege")
        if self.forge == "forgejo" and self.repositories != frozenset(
            {"https://skgit.skstack01.douno.it/smilinTux/sklegal"}
        ):
            raise ReviewPublicationError("connector_repository_scope_mismatch")
