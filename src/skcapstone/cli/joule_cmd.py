...
def register_joule_commands(main: click.Group) -> None:
    ...
    def history_cmd(agent_name: str | None, limit: int, json_out: bool):
        ...
        if json_out:
            click.echo(json.dumps([t.model_dump() for t in txns], indent=2))
            return
        ...
        for txn in txns:
            ...
            amount_str = (
                f"{prefix}{sign}{txn.amount:,}J[/]" if prefix else f"{sign}{txn.amount:,}J"
            )
    ...
    def network_cmd(json_out: bool):
        ...
        if stats.agent_balances:
            ...
            for agent, balance in sorted(stats.agent_balances.items(), key=lambda x: -x[1]):
    ...
    def leaderboard_cmd(json_out: bool):
        ...
        JouleEngine(home=Path(SHARED_ROOT).expanduser())
        ...
        _RANK_COLORS = {1: "bold gold1", 2: "bold grey78", 3: "bold dark_orange3"}  # noqa: N806
        _LEVEL_COLORS = {  # noqa: N806
...
def _resolve_agent(agent_name: str | None) -> str:
    ...
    if agent_name:
        return agent_name
    from .. import SKCAPSTONE_AGENT, active_agent_name
