...
def register_usage_commands(main: click.Group) -> None:
    ...
    @click.option(
        "--date", "date_str", default=None, help="Date to show (YYYY-MM-DD, default: today)."
    )
    ...
    @click.option("--per-day", "do_aggregate", flag_value=False, help="Show per-day breakdown.")
...
def _handle_range(tracker, reports, model, json_out, do_aggregate, label):
    ...
    if do_aggregate:
        ...
        if json_out:
            click.echo(json.dumps([_report_to_dict(r, model) for r in reports], indent=2))
            return
...
def _print_report(report, model_filter: str | None, title_suffix: str = "") -> None:
    ...
    summary = [
        ...
        f"[bold]Est. cost:[/]     [{'green' if total_cost < 0.01 else 'yellow'}]${total_cost:.4f} USD[/]",  # noqa: E501
    ]
