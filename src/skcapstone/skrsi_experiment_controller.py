...
STATES = (
    "hypothesis", "experiment", "intervention_proposal", "evaluation",
    "decision", "rollout_observation", "regression", "recovery",
)
_ALLOWED = {
    None: {"hypothesis"},
...
class ExperimentController:
    ...
    def _prior(self, experiment_id: str, transition_id: str) -> TransitionResult | None:
        for event in self._events(experiment_id):
            if event.get("transition_id") == transition_id and event.get("action") == "experiment_transition":
                return TransitionResult(experiment_id, event["state"], int(event["revision"]), transition_id, True)
        return None
    ...
    def transition(self, experiment_id: str, state: str, *, revision: int,
                   transition_id: str, evidence: Mapping[str, Any],
                   payload: Mapping[str, Any] | None = None,
                   budget: Mapping[str, int] | None = None) -> TransitionResult:
        ...
        if revision != actual_revision + 1:
            raise ExperimentLifecycleError(f"revision fence: expected {actual_revision + 1}, got {revision}")
        ...
        if len([e for e in events if e.get("action") == "experiment_transition"]) >= int(limits["transitions"]):
            raise ExperimentLifecycleError("transition budget exhausted")
        if state == "rollout_observation" and len([e for e in events if e.get("action") == "experiment_evidence"]) >= int(limits["observations"]):
            raise ExperimentLifecycleError("observation budget exhausted")
        common = {"transition_id": transition_id, "revision": revision, "state": state,
        ...
        with card_mutation_lock(home, experiment_id):
            ...
            if state not in _ALLOWED.get(current, set()):
                raise ExperimentLifecycleError(f"invalid transition {current!r} -> {state!r}")
            self.store.append_event(experiment_id, "experiment_transition", self.agent,
                                    **common, payload=dict(payload or {}), proposal_only=True)
            self.store.append_event(experiment_id, "experiment_evidence", self.agent,
                                    transition_id=transition_id, revision=revision,
                                    experiment_id=experiment_id, evidence=dict(evidence))
        return TransitionResult(experiment_id, state, revision, transition_id)

    def regression(self, experiment_id: str, *, revision: int, transition_id: str,
                   evidence: Mapping[str, Any], containment_proposal: Mapping[str, Any],
                   payload: Mapping[str, Any] | None = None) -> TransitionResult:
        result = self.transition(experiment_id, "regression", revision=revision,
                                 transition_id=transition_id, evidence=evidence,
                                 payload={**dict(payload or {}), "containment_proposal": dict(containment_proposal),
                                          "actuation": "forbidden"})
        return result

    def recovery_proposal(self, experiment_id: str, *, revision: int, transition_id: str,
                          evidence: Mapping[str, Any], proposal: Mapping[str, Any]) -> TransitionResult:
        return self.transition(experiment_id, "recovery", revision=revision,
                               transition_id=transition_id, evidence=evidence,
