...
from .seat_boundaries import Action, canonical_principal, require_authority
...
def _principal(identity: str | None) -> str:
    ...
    if not identity:
        raise ReviewPublicationError("card_actor_missing")
    return canonical_principal(identity.lower().removeprefix("capauth:").split("@", 1)[0])
...
def _validate_cards(evidence: SeraphPassEvidence, cards: CardStoreGateway) -> CardSnapshot:
    ...
    if source.verdict not in {"PASS", "PASS_FOR_REVIEW"}:
    ...
    expected = (evidence.repository, evidence.number, evidence.head_sha)
    for card in (source, review):
        if (
            type(card.number) is not int
            or (card.repository, card.number, card.head_sha) != expected
        ):
            raise ReviewPublicationError("card_candidate_binding_mismatch")
    if review.unresolved_review:
        raise ReviewPublicationError("unresolved_review_lineage")
    if (
        not isinstance(source.candidate_evidence_sha256, str)
        or not _SHA256.fullmatch(source.candidate_evidence_sha256)
        or source.candidate_evidence_sha256 != review.candidate_evidence_sha256
    ):
        raise ReviewPublicationError("review_source_evidence_binding_mismatch")
    if _principal(review.reviewer_identity) != "seraph" or _principal(
        review.reviewer_identity
    ) != _principal(evidence.reviewer_identity):
        raise ReviewPublicationError("reviewer_attribution_mismatch")
    if _principal(source.producer_identity) != _principal(review.producer_identity) or _principal(
        source.producer_identity
    ) in {"seraph", "link"}:
        raise ReviewPublicationError("producer_reviewer_not_distinct")
    return review
...
def _validate_live(
    ...
    *,
    ...
    if evidence.repository.startswith("https://") != (capabilities.forge == "forgejo"):
        raise ReviewPublicationError("connector_forge_mismatch")
    ...
    *,
    ...
    with (receipt_dir / f"{key}.lock").open("a+", encoding="utf-8") as lock:
        ...
        if receipt_path.exists():
            approval = _validate_review(existing, evidence, service)
            receipt = _read_receipt(receipt_path)
            if (
                not _receipt_matches(receipt, evidence, service, key)
                or receipt.github_review_id != approval.review_id
            ):
