...
def validate_manifest_invariants(manifest: dict[str, Any]) -> None:
    ...
    if tuple(consumers) != tuple(section_ids):
        raise ValueError("surface consumer contract is missing extra or reordered")
    for surface in surfaces:
        ...
        consumer = consumers[surface["section_id"]]
        if consumer["legacy_fixture_section_id"] != surface["legacy_fixture_section_id"]:
            raise ValueError("surface consumer legacy alias mismatch")
...
def test_manifest_schema_and_all_cross_references_are_closed() -> None:
    ...
    assert manifest["repair_base_commit"] == "d03305180c050f90243a63dee866cf657612f033"
    assert manifest["source_review_card"] == "b417bc62"
    assert manifest["independent_rereview_card"] == "cf83cb57"
...
def test_canonical_manifest_matches_the_exact_18_react_surfaces() -> None:
    ...
    for match in re.finditer(r"<Surface\s+(?P<attrs>.*?)>", source, flags=re.DOTALL):
        attributes = match.group("attrs")
        section_match = re.search(r'id="([a-z0-9-]+)"', attributes)
        if section_match is None:
            continue
        state_match = re.search(r'state="([a-z-]+)"', attributes)
        rendered_states[section_match.group(1)] = (
            state_match.group(1) if state_match is not None else "available"
        )
    assert rendered_states == react_surface_contracts()
...
def test_legacy_fixture_is_reconciled_without_merging_canonical_surfaces() -> None:
    ...
    contracts = legacy_fixture_surface_contracts()
    assert tuple(contracts) == fixture_ids
    for row in matrix["sections"]:
        expected = contracts[row["sectionId"]]
        assert row["component"] == expected["component"]
        assert row["apiContract"] == expected["api_contract"]
        assert row["expectedState"] == expected["expected_state"]
        assert row["contractState"] == expected["contract_state"]
...
def test_claim_identity_has_one_lifecycle_owner_and_one_to_one_projection() -> None:
    ...
    ledger_schema = schema["properties"]["ledger_projection"]
    assert {"tenant_id", "matter_id", "claim_id", "projected_claim_version"}.issubset(
        ledger_schema["required"]
    )
    assert "status" not in ledger_schema["properties"]


def valid_claim_projection() -> dict[str, Any]:
    ...
    return {
        ...
        "statement": "The public-synthetic record supports the stated element.",
        "issue_id": "10000000-0000-4000-8000-000000000004",
        "element_ids": ["10000000-0000-4000-8000-000000000005"],
        "evidence_item_ids": ["10000000-0000-4000-8000-000000000006"],
        "authority_ids": [],
        "ledger_projection": {
            "tenant_id": "10000000-0000-4000-8000-000000000002",
            "matter_id": "10000000-0000-4000-8000-000000000003",
            "claim_id": claim_id,
            "projected_claim_version": 4,
            "projected_claim_status": "accepted",
            "legacy_migration": {
                "legacy_ledger_claim_id": "10000000-0000-4000-8000-000000000007",
                "legacy_status": "supported",
                "mapped_claim_status": "accepted",
                "mapping_revision": "ledger-claim-to-claim-v1",
            },
...
@pytest.mark.parametrize(
    ...
    [
        (
            lambda value: value["ledger_projection"].update(
        ...
        (
            lambda value: value["ledger_projection"].update(
                matter_id="10000000-0000-4000-8000-000000000099"
            ),
            "Matter mismatch",
        ),
        (
            lambda value: value["ledger_projection"].update(
                claim_id="10000000-0000-4000-8000-000000000099"
            ),
            "identity mismatch",
        ),
        (
            lambda value: value["ledger_projection"].update(
                projected_claim_version=3
            ),
            "version mismatch",
        ),
        (
            lambda value: value["ledger_projection"].update(
                projected_claim_status="challenged"
            ),
            "status mismatch",
        ),
        (
            lambda value: value["ledger_projection"]["legacy_migration"].update(
                legacy_status="under_review"
            ),
            "status mapping mismatch",
        ),
        (
            lambda value: value["ledger_projection"]["legacy_migration"].update(
                mapped_claim_status="challenged"
            ),
            "status mapping mismatch",
        ),
    ],
)
def test_claim_projection_rejects_identity_version_and_migration_drift(
    mutation: Any, message: str
) -> None:
    value = valid_claim_projection()
    mutation(value)
    with pytest.raises(ValueError, match=message):
        validate_claim_projection(value, load_json(CLAIM_SCHEMA_PATH))


def test_claim_projection_accepts_exact_canonical_mapping() -> None:
    validate_claim_projection(valid_claim_projection(), load_json(CLAIM_SCHEMA_PATH))
    canonical_only = valid_claim_projection()
    canonical_only["ledger_projection"]["legacy_migration"] = None
    validate_claim_projection(canonical_only, load_json(CLAIM_SCHEMA_PATH))
...
def test_operative_approval_requires_complete_validity_not_hash_only() -> None:
    ...
    valid = valid_operative_approval()
    validate_approval_validity(valid, schema)
    ...
    with pytest.raises(ValidationError, match="None is not of type 'string'"):
        validate_approval_validity(hash_only, schema)


def valid_operative_approval() -> dict[str, Any]:
    subject = {
    ...
    return {
        ...
        "status": "approved",
        "subject": subject,
        "current_scope": {"tenant_id": tenant_id, "matter_id": matter_id},
        ...
        "capability_decision": {
            "decision_id": "10000000-0000-4000-8000-000000000008",
            "outcome": "allowed",
            "tenant_id": tenant_id,
            "matter_id": matter_id,
...
@pytest.mark.parametrize(
    "mutation",
    [
        lambda value: value["capability_decision"].update(outcome="denied"),
        lambda value: value["capability_decision"].update(
            tenant_id="10000000-0000-4000-8000-000000000099"
        ),
        lambda value: value["capability_decision"].update(
            matter_id="10000000-0000-4000-8000-000000000099"
        ),
        lambda value: value["capability_decision"]["subject"].update(
            work_product_id="10000000-0000-4000-8000-000000000099"
        ),
        lambda value: value["capability_decision"]["subject"].update(
            version_id="10000000-0000-4000-8000-000000000099"
        ),
        lambda value: value["capability_decision"]["subject"].update(
            version_number=1
        ),
        ...
        lambda value: value["current_scope"].update(
            tenant_id="10000000-0000-4000-8000-000000000099"
        ),
        lambda value: value["current_scope"].update(
            matter_id="10000000-0000-4000-8000-000000000099"
        ),
        lambda value: value["current_subject"].update(
            work_product_id="10000000-0000-4000-8000-000000000099"
        ),
        lambda value: value["current_subject"].update(
            version_id="10000000-0000-4000-8000-000000000099"
        ),
        lambda value: value["current_subject"].update(version_number=1),
        lambda value: value["current_subject"].update(content_sha256="c" * 64),
        lambda value: value.update(
            superseded_by_approval_id="10000000-0000-4000-8000-000000000099",
            superseded_at="2099-01-02T00:00:00Z",
        ),
        lambda value: value.update(
            status="revoked",
            revoker_principal_id="10000000-0000-4000-8000-000000000099",
            revoked_at="2099-01-02T00:00:00Z",
            revocation_rationale="Approval revoked.",
        ),
    ],
)
def test_operative_approval_rejects_every_decision_scope_and_lifecycle_drift(
    mutation: Any,
) -> None:
    value = valid_operative_approval()
    mutation(value)
    with pytest.raises((ValidationError, ValueError)):
        validate_approval_validity(value, load_json(APPROVAL_SCHEMA_PATH))


def test_nonoperative_approval_requires_bounded_revocation_and_supersession() -> None:
    schema = load_json(APPROVAL_SCHEMA_PATH)
    revoked = valid_operative_approval()
    revoked.update(
        status="revoked",
        revoker_principal_id="10000000-0000-4000-8000-000000000099",
        revoked_at="2099-01-02T00:00:00Z",
        revocation_rationale="Approval revoked.",
        operative=False,
        inoperative_reasons=["revoked"],
    )
    validate_approval_validity(revoked, schema)
    revoked["revocation_rationale"] = None
    with pytest.raises(ValidationError):
        validate_approval_validity(revoked, schema)

    superseded = valid_operative_approval()
    superseded.update(
        superseded_by_approval_id="10000000-0000-4000-8000-000000000099",
        superseded_at="2099-01-02T00:00:00Z",
        operative=False,
        inoperative_reasons=["superseded"],
    )
    validate_approval_validity(superseded, schema)
    superseded["superseded_at"] = None
    with pytest.raises(ValidationError):
        validate_approval_validity(superseded, schema)

    denied = valid_operative_approval()
    denied["capability_decision"]["outcome"] = "denied"
    denied.update(operative=False, inoperative_reasons=["capability_denied"])
    validate_approval_validity(denied, schema)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            lambda value: value["surface_consumers"].pop("decision"),
            "missing extra or reordered",
        ),
        (
            lambda value: value["surface_consumers"]["decision"].update(
                legacy_fixture_section_id="wrong"
            ),
