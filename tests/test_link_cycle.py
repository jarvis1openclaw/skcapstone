...
def test_exactly_one_distinct_reviewer_is_recommended() -> None:
    ...
    assert item.reviewer_identity.identity == "review-key"
    assert item.reviewer_identity.host == "chiap02"
    assert item.as_event()["schema"] == "skfleet.pr-review-handoff/v1"


def test_reviewer_selection_is_stable_when_input_order_changes() -> None:
    expected = handoff()
    reordered = recommend_one_reviewer(
        pr(),
        producer=producer(),
        candidates=list(reversed(reviewers())),
        review_card_id="feedface",
        review_card_revision=HASH,
    )
...
def test_niobe_validates_fresh_handoff() -> None:
    ...
    validate_handoff(
        ...
        current_reviewer_identity=item.reviewer_identity,
...
@pytest.mark.parametrize(
    ("field", "value"),
    [
        ("repository", "smilinTux/other"),
        ("pr_number", 2),
        ("head_sha", "d" * 40),
        ("base_sha", "d" * 40),
        ("source_card", "cafebabe"),
        ("review_card_id", "badc0ffe"),
        ("review_card_revision", "d" * 64),
        ("reviewer", "other-reviewer"),
        ("recommender", "other-seat"),
        ("card_generation", "generation-8"),
        ("observation_revision", "d" * 64),
    ],
)
def test_handoff_digest_rejects_every_actuator_field_tamper(field: str, value: object) -> None:
    item = handoff()
    assert item is not None
    with pytest.raises(
        BoundaryError,
        match="recommender changed|digest changed|reviewer name and identity changed",
    ):
        validate_handoff(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"actor": "link"}, "only niobe"),
    ],
)
def test_stale_duplicate_and_non_niobe_handoffs_fail_closed(kwargs: dict, message: str) -> None:
    item = handoff()
    assert item is not None
    arguments = {
        "current_observation": pr(),
...
def test_handoff_rejects_retargeted_reviewer_identity() -> None:
    item = handoff()
    assert item is not None
    replacement = ReviewerIdentity("eligible", "other-key", "chiap03", "new", "/work/new")
    with pytest.raises(BoundaryError, match="reviewer identity changed"):
        validate_handoff(
            item,
            current_observation=pr(),
            current_review_card_revision=HASH,
            current_reviewer_identity=replacement,
            active_review_card=None,
            used_recommendation_ids=set(),
            actor="niobe",
        )
