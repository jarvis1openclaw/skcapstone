...
class TestDedup:
    def test_identical_alert_deduplicated(self, monkeypatch):
        ...
        with patch("urllib.request.urlopen") as mock_urlopen:
            mock_urlopen.return_value.__enter__.return_value = resp
            first = mgr.notify_headless("agent-dark: poll", "no heartbeat", dedup_key="agent-dark:poll")
            second = mgr.notify_headless("agent-dark: poll", "no heartbeat", dedup_key="agent-dark:poll")
