...
| (`find ~/.skcapstone -name '*.asc'` head -1 grep PRIVATE) | **negative result** | n/a | n/a | n/a | **Zero PGP PRIVATE KEY BLOCKs anywhere under `~/.skcapstone`.** Every `.asc` on the box is a `public.asc` or a `*.pub.asc`. |
