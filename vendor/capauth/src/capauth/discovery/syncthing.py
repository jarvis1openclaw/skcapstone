...
class SyncthingDiscovery(DiscoveryBackend):
    ...
    def __init__(
        ...
        self._api_url = (
            api_url if api_url is not None else os.environ.get("SYNCTHING_URL") or cfg_url
        ).rstrip("/")
        self._api_key = (
            api_key if api_key is not None else os.environ.get("SYNCTHING_API_KEY") or cfg_key
        )
